...
switch lower(svsMethod)

    case 'none'
        % Conventional truncated SVD.
        %
        % Keep the first R singular values without shrinkage and set all
        % remaining singular values to zero.

        s_opt = [sv(1:R); zeros(m-R,1)];


    case 'svs1'
        % Frobenius-loss optimal singular-value shrinkage used by the
        % original 2022 fMRI repository.

        % Estimate the matrix aspect ratio for the residual noise matrix.
        if R/m <= 0.05
            beta_hat = beta0;
        else
            beta_hat = (m-R)/(n-R);
        ...
        s_opt(keep) = opt_fro_shrink(sv(keep));


    case 'svs2'
        % Empirical noise-spectrum-based shrinkage from the original
        % 2022 fMRI repository.
        ...
        for ii = 1:r_hat
            ...
            a = ( ...
                sigma_i^2 * eye(n-r_hat) - ...
                sv_noise * sv_noise' ...
                ).^(-1);

            bmat = ( ...
                sigma_i^2 * eye(m-r_hat) - ...
                sv_noise' * sv_noise ...
                ).^(-1);

            D_hat = ...
                1/(n-r_hat) * trace(sigma_i*a) * ...
                1/(m-r_hat) * trace(sigma_i*bmat);

            D_hatp = ...
                1/(n-r_hat) * trace(sigma_i*a) * ...
                1/(m-r_hat) * ...
                trace(-2*sigma_i^2*bmat.^2 + bmat) + ...
                1/(n-r_hat) * ...
                trace(-2*sigma_i^2*a.^2 + a) * ...
                1/(m-r_hat) * trace(sigma_i*bmat);
    ...
    case 'op'
        ...
        %
        % This implements the OP shrinker from:
        %
        %   denoise_optim_SVShrinkage3.m
        %
        % while retaining the 2022 fMRI repository's SSVD rank and noise
        % estimation.
        %
        % In the 2020 implementation, the input matrix is normalized by:
        %
        %   Y / (sqrt(n) * sigma)
        %
        % In this ssvd.m function, the singular values have already been
        % divided by sqrt(n):
        %
        %   sv = singular_values(Y) / sqrt(n)
        %
        % Therefore, the dimensionless singular values for the OP formula
        % are:
        %
        %   y = sv / sigma

        % Preserve the aspect ratio used by the original OP
        % implementation.
        beta_op = beta0;

        % Initialize all output singular values to zero.
        s_opt = zeros(size(sv));

        if isfinite(sigma) && sigma > 0

            % Convert observed singular values to the dimensionless scale
            % expected by the OP shrinker.
            y = sv ./ sigma;

            % Random-matrix detectability edge.
            op_edge = 1 + sqrt(beta_op);

            % Only singular values at or above the detectability edge can
            % produce a real operator-norm estimate.
            keep = y >= op_edge;
            ...
            if any(keep)
                ...
                % Discriminant from the operator-norm optimal shrinker.
                discriminant = ...
                    (y_keep.^2 - beta_op - 1).^2 - ...
                    4*beta_op;

                % Protect against tiny negative values caused solely by
                % floating-point roundoff at the spectral edge.
                discriminant = max(discriminant,0);

                % Estimate the underlying signal singular values on the
                % dimensionless scale.
                x_squared = 0.5 * ( ...
                    y_keep.^2 - beta_op - 1 + ...
                    sqrt(discriminant));

                x_keep = sqrt(max(x_squared,0));

                % Restore the noise scale used by the current ssvd.m
                % implementation.
                s_opt(keep) = sigma .* x_keep;

            end

        elseif sigma == 0

            % The 2022 rank/noise estimator uses sigma=0 for its special
            % near-zero/no-estimable-noise case. Division by sigma would
            % be undefined.
            %
            % Preserve the observed singular values in this exceptional
            % branch rather than create NaN or Inf.
            s_opt = sv;

        else

            error('Invalid noise estimate sigma=%g in OP shrinkage.', ...
                sigma);
    ...
    otherwise
        ...
        error( ...
            ['Unknown singular-value shrinkage method: %s. ', ...
             'Available methods are none, svs1, svs2, and op.'], ...
            svsMethod);
